...
/*ETHERCAT homing routine: Length of time from the homing signal completed
  and when the counter is reset */
#define COUNTER_RESET 1.000
...
/* 'ethercat_sync_pos' is called from ETHERCAT_HOME_WAIT and loops the 
servo's feedback position to joints command position. This may need to
be moved to HOME_ETHERCAT_WAIT. */
static void ethercat_sync_pos(emcmot_joint_t * joint){
    double sync_temp;
    
    sync_temp = joint->motor_pos_fb;
    joint->motor_pos_cmd = sync_temp;
    joint->pos_cmd = sync_temp;
    joint->pos_fb = sync_temp;
    joint->free_tp.pos_cmd = sync_temp;
    joint->free_tp.curr_pos = sync_temp;
    //joint->free_tp.enable = 1;
...
void do_homing(void)
{
    ...
    do {
        ...
        joint->home_pause_timer = 0;
                ...
                if (joint->home_flags & HOME_AUTO_SERVO){
                    joint->home_state = HOME_ETHERCAT;
                    immediate_state = 1 ;
                }
            break;
        ...
        SET_JOINT_ETHERCAT_EXT_HOMING_FLAG(joint, 0);
        joint->home_state = HOME_IDLE;
        immediate_state = 1;
        ...
        /* Start the servo's internal homing routine. Amplifier should already be in mode 6 HOMING.
           Note: Only an estop will interrupt the homing routine once it has started*/
        case HOME_ETHERCAT:
        joint->free_tp.enable = 1;
        SET_JOINT_ETHERCAT_EXT_HOMING_FLAG(joint, 1);
        // reset the delay timer and jump states
        joint->home_pause_timer = 0;
        joint->home_state = HOME_ETHERCAT_WAIT;
        immediate_state = 1;
        break;

        case HOME_ETHERCAT_WAIT:
        /*  The homing completed signal from the amplifier is a pulse and is not always detected
            reliably. Connecting the homing completed signal to a classicladder input, then connectting
            the output coil to joint.N.ethercat-ext-homed ensures that the signal is high for at least 
            one servo period. 
            Also, there is a delay from when the homing completed signal is sent and when the servo's 
            counter resets to zero. This can be handled in one of two ways. 1) In classic ladder add a 
            timer to delay the signal or 2) Handle it here if you can...*/
        if (joint->ethercat_servo_is_homed){
            joint->home_state = HOME_ETHERCAT_DONE;
            immediate_state = 1;
            break;
        }
        
        ethercat_sync_pos(joint);
