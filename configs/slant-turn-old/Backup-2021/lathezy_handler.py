...
########################################################################
    ...
    def on_cbtn_show_man_spindle_toggled(self,widget,data=None):
        if self.widgets.cbtn_show_man_spindle.get_active():
            print "hiding spindle frame"
            self.widgets.man_spindle_frame.hide()
        else:
            print "showing spindle frame"
...
########################################################################
    ...
    def on_btn_homex_clicked(self,widget,data=None):
        ...
        # Switch amplifier to 0x06 HOMING mode
        self.command.mode(linuxcnc.MODE_MDI)
        self.command.wait_complete()
        self.command.mdi("M204")
        self.command.wait_complete()
        self.command.mode(linuxcnc.MODE_MANUAL)
        self.command.wait_complete()
        ...
        # 
        if self.joint_homing_error == 0:
            # Switch amplifier to CSP Mode
            self.command.mode(linuxcnc.MODE_MDI)
            self.command.mdi("M205")
            self.command.wait_complete()
            self.command.mode(linuxcnc.MODE_MANUAL)        
    ...
    def on_btn_homez_clicked(self,widget,data=None):
        ...
        if self.status.joint[0]["homed"] == 1:
            ...
            # Set z-axis as the active joint
            self.active_joint = 1

            # Switch Amplifier to 0x06 Homing Mode
            self.command.mode(linuxcnc.MODE_MDI)
            self.command.wait_complete()
            self.command.mdi("M214")
            self.command.wait_complete()
            ...
            # When using the internal homing routine of the servo amps, issuing the home command sets 
            # Linuxcnc's homing pin true. Only an E-stop will abort the homing routine
            # ...at this point. 
            self.command.home(1)
            self.wait_for_home()
            
            
            # TBD: take care of homing errors
            if self.joint_homing_error == 0 : 
                self.command.mode(linuxcnc.MODE_MDI)
                self.command.wait_complete()
                self.command.mdi("M215")     
                self.command.wait_complete()    
                self.command.mode(linuxcnc.MODE_MANUAL)
                self.command.wait_complete()
            ...
            message = _("X-Axis must be homed first")
            self.gscreen.warning_dialog(message, True)
    ...
    def jog_disable(self):
        self.widgets.tbtn_activate_jog.set_active(False)
        ## this led might be able to be connected to a linuxcnc pin...
        self.widgets.led_jog_active.set_active(False) 
        self.widgets.jog_button_label.set_label("JOG MODE OFF")               
        self.active_jog_flag = 0
        

    """ Function to delay the code while the servo's have control during the homing routine. 
        If the routine times out we need to abort! """
    def wait_for_home(self):
        start_time = time.time()
        active_joint = self.active_joint
        
        if active_joint == 0:
            timeout = self.joint0_timeout
        if active_joint == 1:
            timeout = self.joint1_timeout

        
        while (time.time() - start_time) < timeout:
            self.joint_homing_error = 0
            self.status.poll()
            if self.status.joint[active_joint]["homed"]:
                return
            time.sleep(0.1)
        
        # If we get here, it means that the homing routine has timed out
        # and we need to raise an error so that the rest of the homing routine does not complete   
        self.joint_homing_error = 1     
        # Throw up a error message so that we know something went wrong.
        message = _("There was an error during the homing routine. Joint %s" % active_joint)
        self.gscreen.warning_dialog(message, True)
        # Abort the routine!      
        self.command.abort()
...
########################################################################
    ...
    def on_btn_change_view_clicked(self,widget):
...
# WORKLIGHT, COOLANT, HSSC use HAL pins
    ...
    def on_tbtn_ignore_limits_toggled(self,widget,data=None):
        if  self.widgets.tbtn_ignore_limits.get_active():
...
######################################################################## 
    ...
    def update_led_ignore_limits(self):
...
########################################################################
    ...
    """ DRO is conatined within an eventbox that does nothing but, the background can be set
        to the desired colour easily, is resizable and looks like the original CRT screen """        
    def set_dro_bg(self):
        self.widgets.eventbox1.modify_bg(gtk.STATE_NORMAL, gtk.gdk.Color("black"))
