...
class HandlerClass:
    ...
    def __init__(self, halcomp,builder,useropts,gscreen):
       ...
       #lathe specific
        ...
        #machine units
        self.data.dro_units = self._IMPERIAL
        self.data.machine_units = self._IMPERIAL
        self.data.jog_increments = 0 
        self.jog_increment_units = self._IMPERIAL
    ...
    def on_rbtn_mdi_toggled(self,widget,data=None):
        ...
        print "mode set to MDI"
                
    def on_rbtn_auto_toggled(self,widget,data=None):
        # paranoid check to see if spindle is running
        self.status.poll()
        spindle_status = self.status.spindle[0]
        spindle_direction = spindle_status['direction']
        print "spindle direction:", spindle_direction
    ...
    def ok_to_switch_modes(self):
        ...
        if self.status.interp_state == linuxcnc.INTERP_IDLE:
            ...
            message = _("The Interpreter is busy BITCH!")
    ...
    def hide_frames(self): 
        ...
        print "self.status.task_mode = %s" % self.status.task_mode
        # MDI = 1, MAN = 3, AUTO = 2 
        # Hide the appropriate frames/boxes/tabs for each specific buttons
    ...
    # jogging and homing are both done in free mode     
    def on_tbtn_activate_jog_toggled(self,widget,data=None):
        ...
        if self.widgets.tbtn_activate_jog.get_active():
            ...
            print "jog flag = %f" % self.active_jog_flag
    ...
    # requires an external mpg
    #TODO populte combobox from ini file settings, but for now just set the jog increment
    #TODO set increment based on G20 or G21
    def on_jog_inc_combobox_changed(self, combobox, data=None):
        print "combobox float value = %f" % (combobox.hal_pin_f.get())  
        if self.jog_increment_units == 0:
            self.jog_increment = combobox.hal_pin_f.get() 
        if self.jog_increment_units == 1:
            scale = (1/25.4)
            # 0.001 mm is the minimum increment for metric mode
            min_metric_inc = combobox.hal_pin_f.get()
            if min_metric_inc >= 0.001:
                self.jog_increment = (min_metric_inc*scale)
            else: 
    ...
    def on_combobox_jog_inc_changed(self,combobox, data=None):
        print "new combobox float value = %f" % (combobox.hal_pin_f.get())
        self.jog_increment = combobox.hal_pin_f.get()
        #jog_increment = combobox.hal_pin_f.get()
        # before we set the jog scale on the mpg we need to check a few things
        # poll the status channel 
        self.status.poll()
        # see if we are in metric mode or imperial mode
        if "G20" in self.data.active_gcodes:
            print "Imperial Mode"
        if "G21" in self.data.active_gcodes:
            print "Metric Mode"
        # see if we are in radius or diameter mode
        if "G7" in self.data.active_gcodes:
            print "Diameter Mode"
        if "G8" in self.data.active_gcodes:
            print "Radius Mode"                                                
        print ("jog increment set to %f") % self.jog_increment
...
# Manual Spindle Frame Buttons ---------------------------------------------------------------------
    ...
    def on_set_spindle_sbutton_value_changed(self,widget,data=None):
        ...
        rpm_request = widget.hal_pin_f.get()  
        self.data.spindle_rpm_request = rpm_request
    ...
    #def on_spindle_rev_rbutton_toggled(self,widget,Data=None):
        ...
        #    self.emc.spindle_reverse(1,self.data.spindle_rpm_request)
        #else:
    ...
    def on_set_spindle_rpm_entry_return(self,widget,result,calc,rpm,userdata2):
        if result == gtk.RESPONSE_ACCEPT:
            ...
            if value >= self.spindle_max_rpm:
               ...
               print "max spindle rpm =" , self.spindle_max_rpm
               print "min spindle rpm =" , self.spindle_min_rpm
               print "calculator value=" , value
               self.spindle_rpm_request = value 
               print "requesting spindle rpm", self.spindle_rpm_request
        # update the label
    ...
    def on_change_view_button_clicked(self,widget):
        # Since this control is for a CNC back tool lathe we only want to use
        # views P & Y2 (0,2) 
        self.data.view = self.data.view + 2
    ...
    def on_show_offsets_button_toggled(self,widget):
        if self.widgets.show_offsets_button.get_active():
            self.widgets.gremlin.show_offsets = 1
        else:
            self.widgets.gremlin.show_offsets = 0            
        print "show gremlin offsets"
    ...
    def on_tbtn_dryrun_toggled(self,widget):
        ...
        if self.widgets.tbtn_dryrun.get_active():
            print "dry run button toggled"
            self.widgets.rapid_override.set_value(float(10.0))
            self.widgets.feed_override.set_value(float(10.0))
                    
        else:
            print "dry run button toggled off"
    ...
    def on_x_offset_origin_entry_return(self,widget,result,calc,axis,userdata2):
        if result == gtk.RESPONSE_ACCEPT:
            ...
            if value == None:
                ...
                axis = "x"
                 #retieve the current tool number
                self.status.poll()
    ...
    def on_btn_index_tool_clicked(self, widget):
        # get the value of the current tool
        self.status.poll()
        current_tool = int(abs(self.status.tool_in_spindle))
        print "current tool", current_tool
        # get the value of the combobox
        request_tool = int(abs(self.widgets.cbox_turret.hal_pin_f.get()))
        print "requested tool", request_tool
    ...
    # keyboard input
    # ideally we could use gscreens built in method launch_keyboard() but once onboard is open it is 
    # hard to close and it keeps opening a new version, so we're going to KISS it...Keep It Simple Stupid
    # USER is responsible for closing keyboard when finished
    # might want to add a couple of checks to see if the interpreter is busy etc    
    ...
    def launch_keyboard(self,widget,args="",x="",y=""):
        ...
        if self.status.task_mode == linuxcnc.MODE_MDI:
            ...
            #if were in manual mode kill it 
            p.kill()

    # embeded keyboard         
    def on_tbtn_keyboard_toggled(self,widget,args="",x="",y=""):
        if self.widgets.tbtn_keyboard.get_active():
    ...
    # MDI remapped M code buttons are called using toggle action MDI action widgets

    # convience function to check if it is ok to issue mdi commnad
    def ok_for_mdi(self):
        self.status.poll()
        return not self.status.estop and self.status.enabled and (self.status.interp_state == linuxcnc.INTERP_IDLE)        
...
########################################################################
# Special callback for homing. 
# When the joint.xxx.homing pin = True, LinuxCNC can only accept an E-Stop
# This call back has complete control... BE CAREFUL
# See modified homing.c file for details

    def _on_home_x_req_changed(self,pin,userdata=None):
...
# LED's section---------------------------------------------------------
    ...
    def update_all(self):
        #convience function to update everything, called every 100ms 
        #self.update_jog_velocity_label()
        self.status.poll()
        self.update_file_label()
        self.update_crt_dro()
        self.update_mpg_scale()
        #self.update_lbl_set_rpm()
    ...
    def _set_spindle(self,command):
        ...
        print "override value =" ,  rpm_override
        try:
            #rpm_out = rpm * self.status.spindle[0]['override']
            rpm_out = rpm
        except:
            rpm_out = 0
        print " rpm:" , rpm_out
...
########################################################################
	...
	def initialize_preferences(self):	
        ...
        # the scale is weird since the spped control widget has a default of 60
        ...
        self.widgets.feed_override.set_property("increment",float(2.5))
    ...
    def initialize_pins(self):
        ...
        # gear-change pins
        #self.gscreen.halcomp.newpin("gear-in-neutral", hal.HAL_BIT, hal.HAL_IN)
        
        # set this pin True to signal a manual tool change and that the coordinate 
        # & wear off-sets should NOT be applied
        self.gscreen.halcomp.newpin("manual-tool-change", hal.HAL_BIT, hal.HAL_OUT)
        
        # Use this pin to call a custom m-code to home the amplifiers
        self.home_x_req = hal_glib.GPin(self.gscreen.halcomp.newpin('home-x-req', hal.HAL_BIT, hal.HAL_IN))
        self.home_x_req.connect('value-changed',self._on_home_x_req_changed)
