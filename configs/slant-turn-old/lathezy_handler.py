...
# WORKLIGHT, COOLANT, HSSC use HAL pins
    ...
    def on_tbtn_hssc_toggled(self,widget,data=None):
        """ Function to determine the values of the HSSC spinbuttons. 
        Then call M600 Pxxx Qxxx Rxx if the button is active or M601 
        to turn off HSSC. This can only be issued in mdi/auto
        mode 
        """
        if self.widgets.tbtn_hssc.get_active():
            #for now just try to figure out the value of the spin buttons
            per = self.widgets.spin_btn_period.get_value()
            amp = self.widgets.spin_btn_amplitude.get_value()
            dur = self.widgets.spin_btn_duration.get_value()
            print "Period = %f" % per
            print "Amplitude = %f" % amp
            print "Duration =%f" % dur
            
            self.command.mdi(("M600 P%f Q%f R%f")% (per,amp,dur))
            
            
        else:
            print "HSSC toggled off"
