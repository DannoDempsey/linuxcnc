...
class StatCanon(glcanon.GLCanon, interpret.StatMixin):
    ...
    # This might be a mistake, is_lathe() returns self.lathe_option ...
    #def is_lathe(self):return self.lathe_view_option
    """
    def is_lathe(self): 
        print("Lathe Option:", self.lathe_view_option)
...
class Lcnc_3dGraphics(QOpenGLWidget,  glcanon.GlCanonDraw, glnav.GlNavBase):
    ...
    def __init__(self, parent=None):
        ...
        self.show_tool = True
        # Lathe specific variables for DRO display
        self.show_lathe_radius = False
        self.show_lathe_css = False
        self.show_lathe_vel = False
        self.show_dtg = True
        self.grid_size = 0.0
    ...
    def poll(self):
        ...
        # Track the status of 
        #   - G7/G8 display shows  Dia. or Rad.
        #   - G96/G97 CSS mode or RPM for lathes
        #   - G00 display velocity or feed rate
        current_gcodes = s.gcodes
        self.show_lathe_radius = 80 in current_gcodes
        self.show_lathe_css = 960 in current_gcodes
        self.show_lathe_vel = 00 in current_gcodes
        ...
        fingerprint = (self.logger.npts, self.soft_limits(),
            ...
            s.motion_mode, s.current_vel, self.show_lathe_radius, self.show_lathe_css,self.show_lathe_vel)
    ...
    # This function seems to be called continuiously instead of being set from the ini file 
    # and then used in dro_format() 
    ...
    def dro_format(self,s,spd,dtg,limit,homed,positions,axisdtg,g5x_offset,g92_offset,tlo_offset):
            ...
            if self.metric_units:
                #Display a blank string at least 6 characters long(s = axis) + dro_mm
                format = "% 6s:" + self.dro_mm
                if self.show_dtg:
            ...
            # Display G5x and G92 Offsets. 
            # If the machine is a lathe don't display G54 Rotational or G92 Offsets
            for i in range(9):
                if self.is_lathe() and i ==1:
                    continue
                index = s.g5x_index
                ...
                if s.axis_mask & (1 << i):
                    if self.is_lathe():
                        if self.metric_units:
                            # Show only G5x offset, skip G92 offset
                            droposstrs.append(f"{label} {a}: {self.dro_mm % g5x_offset[i]}")
                        else:
                            droposstrs.append(f"{label} {a}: {self.dro_i % g5x_offset[i]}")
                    else:
                        # Show both G5x and G92 offsets for mills
            ...
            # Show Velocity/Feed Rate, RPM, SFM
            if self.show_velocity:
                ...
                if self.is_lathe():
                    if self.metric_units:
                        ...
                        if self.spindle_speed != 0 and not self.show_lathe_vel:
                    ...
                    # G96 CSS mode is active, show feed, RPM, CSS 
                    if self.show_lathe_css:
                        posstrs.append("{} RPM: {} {}".format(feed,self.spindle_speed,sf))
                    
                    # G97 RPM mode is active, show feed RPM
                    else: 
            ...
            # Ensure DTG is displayed for each active axis, not just once
            if self.show_dtg:
                for i in range(9):
                    if s.axis_mask & (1 << i):
                        a = "XYZABCUVW"[i]
