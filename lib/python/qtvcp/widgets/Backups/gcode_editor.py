...
class EditorBase(QsciScintilla):
    ...
    def __init__(self, parent=None):
        ...
        # G-code highlight line
        self.userHandle = self.markerDefine(QsciScintilla.Background, self.USER_MARKER_NUM)
        self.setMarkerBackgroundColor(QColor("yellow"), self.CURRENT_MARKER_NUM)
        ...
        #'GcodeDisplay' object has no attribute 'ensureLineVisable'
        #self.ensureLineVisable(True)
    ...
    """
    # New method for on_margin_clicked
    def on_margin_clicked(self, nmargin, nline, modifiers):
        # Toggle marker for the line the margin was clicked on
        # 2 means its already there
